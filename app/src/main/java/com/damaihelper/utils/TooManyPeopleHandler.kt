 ...
 * 【修正】将 object TooManyPeopleHandler 改为 class TooManyPeopleHandler(private val context: Context)
 */
class TooManyPeopleHandler(private val context: Context) {

    // 修正：将 const val 改为 val
    private val TAG = "TooManyPeopleHandler"
    ...
    fun detectTooManyPeopleScenario(rootNode: AccessibilityNodeInfo): TooManyPeopleInfo? {
        ...
        // 遍历所有文本节点查找关键词
        val allTextNodes = getAllTextNodes(rootNode)
    ...
    private fun calculateSmartDelay(
        ...
        // 添加随机变化
        // 修正：使用 kotlin.random.Random 而不是 java.util.Random
    ...
    // ========== 辅助工具方法 ==========
    // ... (getAllTextNodes, traverseNodeForText, findAllClickableNodes, traverseNodeForClickable 保持不变)

    /**
     * 获取所有包含文本的节点
     */
    private fun getAllTextNodes(rootNode: AccessibilityNodeInfo): List<AccessibilityNodeInfo> {
        val textNodes = mutableListOf<AccessibilityNodeInfo>()
        traverseNodeForText(rootNode, textNodes)
        return textNodes
    }
     ...
     * 递归遍历节点树查找文本节点
     */
    private fun traverseNodeForText(node: AccessibilityNodeInfo, result: MutableList<AccessibilityNodeInfo>) {
        ...
        for (i in 0 until node.childCount) {
            ...
            if (child != null) {
                traverseNodeForText(child, result)
            }
        }
    }
    ...
    private fun findAllClickableNodes(rootNode: AccessibilityNodeInfo): List<AccessibilityNodeInfo> {
        ...
        traverseNodeForClickable(rootNode, clickableNodes)
     ...
     * 递归遍历节点树查找可点击节点
     */
    private fun traverseNodeForClickable(node: AccessibilityNodeInfo, result: MutableList<AccessibilityNodeInfo>) {
        if (node.isClickable) {
            result.add(node)
        }
        ...
        for (i in 0 until node.childCount) {
            val child = node.getChild(i)
            if (child != null) {
                traverseNodeForClickable(child, result)
            }
        }
    }
}
